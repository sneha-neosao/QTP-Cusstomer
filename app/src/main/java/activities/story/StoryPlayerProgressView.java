...
public class StoryPlayerProgressView extends View {
    ...
    public StoryPlayerProgressView(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init(context);
        TypedArray ta = context.obtainStyledAttributes(attrs, xute.storyview.R.styleable.StoryPlayerProgressView, 0, 0);
        try {
            mProgressHeight = getPxFromDp((int) ta.getDimension(xute.storyview.R.styleable.StoryPlayerProgressView_progressBarHeight, PROGRESS_BAR_HEIGHT));
            mGapBetweenProgressBars = getPxFromDp((int) ta.getDimension(xute.storyview.R.styleable.StoryPlayerProgressView_gapBetweenProgressBar, GAP_BETWEEN_PROGRESS_BARS));
            progressBarPrimaryColor = ta.getColor(xute.storyview.R.styleable.StoryPlayerProgressView_progressBarPrimaryColor, Color.parseColor(PROGRESS_PRIMARY_COLOR));
            progressBarSecondaryColor = ta.getColor(xute.storyview.R.styleable.StoryPlayerProgressView_progressBarSecondaryColor, Color.parseColor(PROGRESS_SECONDARY_COLOR));
            singleStoryDisplayTime = ta.getInt(xute.storyview.R.styleable.StoryPlayerProgressView_singleStoryDisplayTime, SINGLE_STORY_DISPLAY_TIME);
        } finally {
            ta.recycle();
        }
        prepareValues();
